...
const int N_marker = 2;
double detected_pos[N_marker][3] = {{0.0, 0.0, 0.0}};
...
private:
  ...
  double centroid_xyz[N_marker][3];
  std::string sample_path;
  float color_idx[N_marker][3];
  float color_MIN[N_marker][3];
  float color_MAX[N_marker][3];
  cv::Mat hist_2D;
...
private:
  void start(const Mode mode)
  {
    ...
    for(int i_marker = 0; i_marker<N_marker; i_marker++)
    {
      
      std::string folder("/home/motion/ros_ws/src/iai_kinect2/kinect2_viewer/data/sample/sample00");
      std::string fileEnd(".jpg");
      std::string filecount = to_string(i_marker+1);



      this->sample_path = folder + filecount + fileEnd;
      std::cout << this->sample_path<<endl;
      ...
      for(int i = 0; i < 3; i++)
      {
        ...
        this->color_idx[i_marker][i] = max_loc.y;
      }

      for(int i = 0 ; i<3; i++)
      {
        this->color_MAX[i_marker][i] = this->color_idx[i_marker][i] + hsv_threshold[i];
        this->color_MIN[i_marker][i] = this->color_idx[i_marker][i] - hsv_threshold[i];
        cout<< "color_idx["<< i << "] = " << color_idx[i_marker][i] << endl;
        cout<< "color_range ["<< i << "] = [" << color_MIN[i_marker][i] << ", " << color_MAX[i_marker][i] << "]" << endl;
  ...
  void poseCallback(const visualization_msgs::MarkerArray markerArray)
  {
    static tf::TransformBroadcaster br;
    for(int i_marker=0; i_marker<N_marker;i_marker++)
    {  
      tf::Transform transform;
      transform.setOrigin( tf::Vector3(markerArray.markers[i_marker].pose.position.x, markerArray.markers[i_marker].pose.position.y, markerArray.markers[i_marker].pose.position.z));
      tf::Quaternion q;
      q.setRPY(0.0, 0.0, 0.0);
      transform.setRotation(q);
      std::string markerPosTrans = "markerPos_" + to_string(i_marker);
      br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "kinect2_link", markerPosTrans));
    }
  }
    ...
    for(; running && ros::ok();)
    {
      if(updateCloud)
      {
        ...
        for(int i_marker=0; i_marker<N_marker; i_marker++)
        {
          ...
          markerArray.markers[i_marker].pose.position.x = detected_pos[i_marker][0];
          markerArray.markers[i_marker].pose.position.y = detected_pos[i_marker][1];
          markerArray.markers[i_marker].pose.position.z = detected_pos[i_marker][2];
        }

        pub_MarkerArray.publish(markerArray);
        poseCallback(markerArray); 
  ...
  void colorSegmentation(const cv::Mat &color, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &cloud) const
  {
    for(int i_marker=0; i_marker<N_marker; i_marker++)
    {
      ...
      cv::inRange(color_hsv, cv::Scalar(this->color_MIN[i_marker][0], this->color_MIN[i_marker][1], this->color_MIN[i_marker][2]), cv::Scalar(this->color_MAX[i_marker][0], this->color_MAX[i_marker][1], this->color_MAX[i_marker][2]), mask);
      ...
      for(int i = 0; i<3; i++)
      {
        ...
        centroid_xyz[i] = centroid_xyz[i]/counter;
        detected_pos[i_marker][i] = centroid_xyz[i];

      }
      printf("detected position %d  = [%f, %f, %f] \n", i_marker, detected_pos[i_marker][0], detected_pos[i_marker][1], detected_pos[i_marker][2]);
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "MarkerPosition");
  ros::NodeHandle n;
  pub_MarkerArray = n.advertise<visualization_msgs::MarkerArray>("MarkerPosition", 10);

  marker.header.frame_id = "/kinect2_link";
  marker.header.stamp = ros::Time::now();

  marker.ns = "MarkerPosition";
  ...
  for(int i_marker=0; i_marker<N_marker; i_marker++)
  {
    ...
    marker.id = i_marker;
    marker.pose.position.x = detected_pos[i_marker][0];
    marker.pose.position.y = detected_pos[i_marker][1];
    marker.pose.position.z = detected_pos[i_marker][2];
  ...
  ros::init(argc, argv, "multicolor_kinect2_position_tracking", ros::init_options::AnonymousName);
